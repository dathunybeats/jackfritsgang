...
# JackFrits AWS Setup Script for Linux/macOS
echo "Setting up AWS resources for JackFrits Video Text Overlay..."
echo

# Set variables
BUCKET_NAME="jackfrits-video-bucket-$(date +%s)"
...
sed "s/BUCKET_NAME_PLACEHOLDER/$BUCKET_NAME/g" bucket-policy.json > bucket-policy-final.json
...
cd ../aws-lambda
npm install
zip -r ../aws-setup/lambda-function.zip .
cd ../aws-setup
